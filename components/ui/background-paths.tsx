...
function FloatingPaths({ position }: { position: number }) {
    // Deterministic, seeded duration per path (no impure calls during render)
    const seededDuration = (n: number) => {
        const v = Math.sin(n * 12.9898) * 43758.5453;
        const frac = v - Math.floor(v);
        return 20 + frac * 10; // 20s to 30s range
    };
...
// Simple demo wrapper for quick usage
